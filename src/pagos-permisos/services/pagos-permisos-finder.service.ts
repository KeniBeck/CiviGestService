...
export class PagosPermisosFinderService {
  ...
  /**
   * Buscar pagos por documento del ciudadano (PÚBLICO - sin autenticación)
   * Permite a los ciudadanos consultar sus propios comprobantes
   */
  async findByDocumento(documento: string) {
    const pagos = await this.prisma.pagoPermiso.findMany({
      where: {
    ...
    if (pagos.length === 0) {
      throw new NotFoundException(
        'No se encontraron pagos para el documento proporcionado',
      );
    }
